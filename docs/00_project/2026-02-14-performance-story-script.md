...
안녕하세요. 스프링부트 백엔드 개발자로서,
기능 구현 이후 성능 병목을 데이터로 검증하고 개선하는 과정을 중요하게 생각합니다.
유치원 ERP 프로젝트에서 알림장과 대시보드 병목을 직접 측정하고,
조회 경로/집계 쿼리/인덱스를 개선해 실제 수치로 성능을 낮춘 경험이 있습니다.

## 40초 성능 사례 (핵심 답변)

대표 사례는 알림장 목록 API였습니다.
초기에는 읽음 수를 건별 조회하는 N+1 패턴 때문에 페이지 조회 시 쿼리가 크게 늘어났습니다.
읽음 수를 다건 집계 쿼리로 바꿔 쿼리 수를 22에서 4로 줄였고,
동일 시나리오에서 응답 시간도 15ms에서 4ms로 개선했습니다.

이후 대시보드 통계도 목록 로딩 기반 계산을 집계 쿼리로 통합해
13쿼리에서 10쿼리, 14ms에서 2ms로 줄였습니다.
마지막으로 EXPLAIN 기반 인덱스 튜닝을 적용해
ALL/filesort 실행계획을 ref/range 중심으로 개선했습니다.
