...
- 결정: 원장 전용 `/api/v1/notification-outbox` 운영 API와 `/notification-outbox` 화면을 추가해 summary, 채널 필터, dead-letter 목록, 수동 retry를 제공합니다.
- 검증: 원장 성공, 교사 접근 차단, 채널별 dead-letter 필터, dead-letter 재시도 상태 전이, view endpoint 접근을 통합 테스트로 확인했습니다.
...
- Notification Outbox 운영 화면은 dead-letter 중심이며 채널 필터까지 제공합니다. 전체 outbox timeline은 후속 개선입니다.
