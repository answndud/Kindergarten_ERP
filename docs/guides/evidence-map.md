...
| Notification Outbox dead-letter를 운영자가 확인/채널 필터/재시도할 수 있다. | `src/main/java/com/erp/domain/notification/controller/NotificationOutboxOpsController.java`, `src/main/java/com/erp/domain/notification/*` | `src/test/java/com/erp/api/NotificationOutboxOpsApiIntegrationTest.java`, `src/test/java/com/erp/integration/ViewEndpointTest.java` | `/notification-outbox`, `README.md` 화면 섹션 | demo에서 outbox 화면을 열고 채널 필터와 principal-only 테스트로 이동 |
...
| 알림 Outbox 운영 | `/notification-outbox` | dead-letter summary/channel filter/list/retry | `NotificationOutboxOpsApiIntegrationTest`, `ViewEndpointTest` |
