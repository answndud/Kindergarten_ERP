...
| Notification Outbox timeline과 dead-letter를 운영자가 검색/필터/재시도할 수 있다. | `src/main/java/com/erp/domain/notification/controller/NotificationOutboxOpsController.java`, `NotificationOutboxOpsService`, `NotificationChannelSenderRegistry`, `src/main/java/com/erp/domain/notification/service/channel/*Sender.java` | `src/test/java/com/erp/api/NotificationOutboxOpsApiIntegrationTest.java`, `src/test/java/com/erp/integration/ViewEndpointTest.java` | `/notification-outbox`, `README.md` 화면 섹션 | demo에서 outbox timeline의 status/channel/q 필터와 principal-only 테스트로 이동 |
| 외부 알림 provider는 sender adapter 경계 뒤에 둔다. | `NotificationChannelSender`, `NotificationChannelSenderRegistry`, `EmailNotificationSender`, `PushNotificationSender`, `IncidentWebhookNotificationSender`, `AppNotificationSender` | `NotificationOutboxOpsApiIntegrationTest`, notification outbox integration tests | `docs/guides/risk-response.md`, `docs/guides/interview-guide.md` | 실제 provider 미연동 질문에서 adapter 경계와 운영 전 보완책을 함께 설명 |
...
| 클라우드 미배포 상태에서도 production-like dry-run 증거가 있다. | `deploy/docker-compose.prod.yml`, `deploy/.env.prod.example`, `Dockerfile` | Startup safety/observability tests, compose config, `bootJar` | `docs/guides/production-like-checklist.md`, `docs/guides/deployment-guide.md` | 미배포 질문에서 dry-run checklist와 외부 의존성 목록을 제시 |
