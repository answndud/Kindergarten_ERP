...
| 외부 알림은 실제 발송이 아니라 시연/mocking 중심 아닌가요? | outbox 상태 전이, retry/backoff/dead-letter, 채널 필터와 운영 API 중심 | 외부 provider 비용/계정 없이도 실패 대응과 재처리 설계를 검증하기 위함 | 실제 provider adapter, webhook signature, DLQ 알림, provider별 rate limit 추가 | `domain/notification`, `NotificationOutboxOpsApiIntegrationTest` |
