...
- Redis를 인증의 critical dependency로 보고 readiness/운영 대응을 같이 설계 가능
...
**해결**
- 현재 저장소 기준으로는 `refresh:session:{memberId}:{sessionId}` + `refresh:sessions:{memberId}` 구조로 정리
- 세션 단위 GET/DEL과 세션 집합 조회로 멀티 디바이스 revoke와 회전을 같이 지원
...
초기에는 로그인 정책이 단순해 동시 로그인과 세션 회수 시나리오를 설명하기 어려웠고,
개별 기기 세션을 끊는 기능도 만들기 어려웠습니다.

**해결**
- 세션 단위 키 구조와 세션 메타데이터를 도입해 멀티 디바이스를 기본 지원
- 개별 세션 revoke, 다른 기기 로그아웃, 현재 세션 표시까지 확장
- 클라이언트 에러 메시지를 표준화해 만료/무효 세션 UX를 일관화
...
이를 세션 단위 키 구조(`refresh:session:{memberId}:{sessionId}`), 저장 API 단일화(`set + TTL`),
직렬화 규칙 통일로 정리했습니다.
...
저는 키 규칙, TTL 정책, 장애 시 동작(fail-closed + readiness 하향)을 문서화해
...
현재 저장소는 Redis를 세션 레지스트리 SSOT로 두기 때문에,
Redis가 응답하지 않으면 access/refresh 인증을 fail-closed로 처리합니다.
대신 readiness와 운영 문서에서 Redis를 critical dependency로 관리합니다.
...
Redis 도입 시 세션 단위 키와 세션 집합을 같이 두어 조회/회수를 예측 가능한 경로로 만들고,
...
1) 패턴 검색 키 설계로 인한 비효율을 세션 단위 Redis 키 구조로 개선,
2) TTL 누락을 저장 메서드 단일화로 차단,
3) JDK 직렬화/문자열 혼재 문제를 `StringRedisTemplate`로 통일,
4) 동시 로그인 정책 충돌을 세션 레지스트리 구조로 정리했습니다.
...
- 운영 관점: 세션 레지스트리 정책, 장애 시 fail-closed 대응
...
4. 핵심 설계는 세션 단위 Redis 키와 세션 집합을 함께 두어 조회/회수를 예측 가능한 경로로 만든 것입니다.
...
8. 정책 충돌은 세션 레지스트리와 개별 revoke 정책으로 정리했습니다.
...
7. 이후 키를 세션 단위로 고정하고 저장은 `set + TTL`만 허용하도록 구조를 바꿨습니다.
