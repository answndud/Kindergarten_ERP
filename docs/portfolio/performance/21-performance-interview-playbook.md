...
- 10초 답변:
  패턴 키 검색은 운영에서 부담이 커서, 세션 단위 Redis 키와 세션 집합 구조로 바꿨습니다.
- 30초 답변:
  기존 패턴 검색 방식은 키 수 증가 시 비용과 운영 리스크가 커집니다.
  그래서 세션 단위 키 설계로 조회/회수를 예측 가능한 경로로 만들고,
  재발급 시 Redis 저장 토큰과 요청 토큰의 일치 검증, 개별 세션 revoke까지 같이 지원하게 했습니다.
...
- 피해야 할 답변:
  코드가 단순해져서요.
- 권장 답변:
  핵심은 패턴 검색 제거와 세션 제어입니다.
  세션 단위 키와 세션 집합 구조로 운영 중 키 스캔 리스크를 낮추고,
  토큰 일치 검증과 개별 세션 revoke까지 같이 지원하게 했습니다.
