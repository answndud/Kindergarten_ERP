...
### 운영 결정(2026-02-14)
- 폴링은 프래그먼트별 `every 30s` 선언 대신 `app.js`의 전역 스케줄러로 단일화한다.
- `visibilitychange`/`focus` 시점에 즉시 `notifications-changed` 이벤트를 발행해 복귀 직후 최신 상태를 반영한다.
- 알림 드롭다운을 열 때 강제 갱신 이벤트를 발행해 목록 stale 상태를 줄인다.
...
### 구현 방식
```html
<!-- HTMX 이벤트 기반 갱신 (폴링 트리거는 app.js 전역 스케줄러에서 발행) -->
<div hx-get="/notifications/fragments/badge"
     hx-trigger="load, notifications-changed from:body"
     hx-swap="outerHTML">
    <!-- 안 읽은 알림 수 -->
</div>
```
...
### 이유
1. **실시간성**: 사용자 액션/포커스 복귀 시 즉시 갱신
2. **안정성**: 전역 폴링으로 트리거를 한 곳에서 관리
3. **간단한 구현**: HTMX로 부분 렌더링
...
- 2026-02-14: 전역 폴링 스케줄러로 트리거 단일화 + 포커스/가시성 기반 즉시 갱신 적용
