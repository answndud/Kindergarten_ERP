...
- 2026-01-18: 대시보드 통계용 기간(7/30일) 출석률 계산 기준 확정
- 2026-01-29: 출석 고도화(일괄 처리/사유 입력/월간 리포트) 결정
...
### 결정
교사 업무 흐름을 줄이기 위해 **일괄 처리 + 사유 입력 + 월간 리포트**를 추가한다.
...
### 구현 방식
- API: `/api/v1/attendance/bulk`, `/api/v1/attendance/upsert`, `/api/v1/attendance/classroom/{classroomId}/monthly-report`
- 서비스: Upsert 기반 저장 + 반별 월간 집계
- UI: 출결관리 화면에서 일괄 처리/사유 입력, 별도 월간 리포트 페이지 제공

### 성능 개선 포인트
1. **Upsert 방식**
   - 출석 생성/수정 분리를 없애고 `findByKidIdAndDate` 기반으로 업데이트
   - 중복 데이터 생성 방지 + 호출 단순화
2. **월간 리포트 집계**
   - 반+월 기준 데이터를 한 번에 로드 후 메모리 집계
   - 화면별 요청 수를 줄여 UX 체감 속도 개선
3. **일괄 처리 API**
   - 개별 요청을 줄이고 서버 단일 호출로 배치 처리

### 변경 이력
- 2026-01-29: 출석 고도화 기능 추가
