...
- 본 문서는 초기 단계별 계획 문서이며, 다수 항목은 현재 구현 완료 상태입니다.
- 최신 작업 SSOT는 루트의 `PLAN.md`, `PROGRESS.md`를 우선합니다.
- 현재 운영 중인 핵심 기능: 인증/JWT/OAuth2, 유치원/반/원생, 출석, 알림장, 공지, 지원/승인, 알림, 캘린더, 대시보드.
- 현재 후속 과제는 신규 기능 대규모 추가보다 문서 정합성/권한 회귀 방지/성능 스토리 고도화 중심으로 관리합니다.

### 단기 백로그 (현재 우선순위)

1. 회고 문서 정리 (`docs/retrospective/`)
2. API 계약 변경 시 README/CURRENT_FEATURES 동시 업데이트
3. 권한 경계 API 통합 테스트 지속 보강
4. 성능 최적화 문서의 전/후 지표 갱신
